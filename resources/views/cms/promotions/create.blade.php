...
@section('content')
    <section class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <p class="font-['Host_Grotesk'] text-[12px] font-bold uppercase tracking-[0.35em] text-[#8f5a3a]">New Promotion</p>
                <h1 class="mt-2 font-['Host_Grotesk'] text-[32px] font-bold text-[#241810]">Create Promotion</h1>
            </div>
        ...
        <div class="rounded-[22px] border border-[#dbcdbd] bg-white p-8">
            <form action="{{ route('cms.promotions.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="8" class="w-full px-4 py-3 border border-[#d8c9b7] rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <label class="inline-flex items-center gap-2">
                        <input type="checkbox" name="active" value="1" {{ old('active', true) ? 'checked' : '' }} class="h-4 w-4 rounded border-[#d8c9b7] text-amber-600 focus:ring-amber-500" />
                        <span class="text-[#241810]">Active</span>
                    </label>
                </div>

                <div class="pt-4 border-t border-[#e5dbcf]">
                    <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-6 rounded-lg">Create Promotion</button>
