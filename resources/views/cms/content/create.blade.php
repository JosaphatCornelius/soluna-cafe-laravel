...
@section('content')
    <section class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <p class="font-['Host_Grotesk'] text-[12px] font-bold uppercase tracking-[0.35em] text-[#8f5a3a]">New Content Page</p>
                <h1 class="mt-2 font-['Host_Grotesk'] text-[32px] font-bold text-[#241810]">Create Content Page</h1>
            </div>
        ...
        <div class="rounded-[22px] border border-[#dbcdbd] bg-white p-8">
            <form action="{{ route('cms.content.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="pt-4 border-t border-[#e5dbcf]">
                    <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-6 rounded-lg">Create Page</button>
                    <a href="{{ route('cms.content.index') }}" class="ml-4 inline-block text-[#5d4a3d] hover:text-[#241810]">Cancel</a>
                </div>
            </form>
        </div>
    </section>
